...
if __name__ == "__main__":
    ...
    if CHECKCHROM:
        x = list(pd.read_csv(EXPRFILE, nrows=1, index_col=0).columns.values)
        x = [item for item in x if item == "sample" or CHROM in item]
    ...
    if "sample" in expr.columns:
        expr.index = expr["sample"].astype('str')
        expr = expr.drop("sample", 1)
    ...
    for i in range(expr_annot.shape[0]):
        ...
        for j in range(len(str_ids)):            
            ...
            beta, beta_se, p_linear = LinearRegression(locus_str, locus_y["expr"].values, norm=NORM, minsamples=MINSAMPLES)

            # Write output
            if beta is not None:
                f.write("\t".join(map(str, [gene, CHROM, str_ids[j], starts[j], len(str_samples)-locus_str.shape[0], beta, beta_se, -1, p_linear]))+"\n")
#        break
    f.close()
